...
import { ArrowRight, ArrowLeft, CheckCircle2, XCircle, MinusCircle, Lightbulb, Trophy, Home, RotateCcw } from 'lucide-react';
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-ink-muted italic">Loading PYQ questions...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-lg mx-auto space-y-4 py-8 px-4">
        ...
        <div className="flex gap-3">
          ...
          <Button className="flex-1" onClick={() => router.push('/pyq')}>
            <Home size={18} /> PYQs
          </Button>
        </div>
      </motion.div>
  ...
  if (!q) {
    return (
      <div className="text-center py-20">
        <p className="text-ink-muted italic">No PYQ questions found for {year}.</p>
        <Button variant="ghost" onClick={() => router.push('/pyq')} className="mt-4">
          <ArrowLeft size={18} /> Back to PYQs
        </Button>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center justify-between text-sm text-ink-muted">
        <span>PYQ {year} · {currentIndex + 1}/{questions.length}</span>
